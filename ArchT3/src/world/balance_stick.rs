...
pub struct BalanceStickAnimation {
    ...
    broad_phase: BroadPhase, // Changed from DefaultBroadPhase
...
impl BalanceStickAnimation {
    ...
    pub fn new(duration: f64) -> Self {
        ...
        Self {
            ...
            broad_phase: BroadPhase::new(), // Updated constructor
    ...
    pub fn run(&mut self) -> Vec<f64> {
        ...
        // Fix: Convert NonZeroUsize to usize (or wrap correctly depending on Rapier version)
        // Here we assume standard assignment compatibility or .into()
        integration_parameters.num_solver_iterations = std::num::NonZeroUsize::new(12).unwrap().into();

        for _step in 0..num_steps {
            // 1. Détermination de la force appliquée (u)
            let mut applied_force: Real = 0.0;
            ...
            // 2. Step de physique
            // Fix: Removed the explicit `None` for query_pipeline to match the 12-argument requirement
            self.physics_pipeline.step(
                ...
                &mut self.ccd_solver,
                // None, <--- Removed this argument based on compiler error
                &(),
                &(),
            );
            
            // Optional: Update query pipeline manually if needed
            ...
            if let (Some(cart), Some(stick)) = (
                ...
                let stick_av = stick.angvel().x as f64; 

                data_stream.push(cart_x);
                data_stream.push(stick_angle); 
                data_stream.push(cart_v);      
                data_stream.push(stick_av);     
                data_stream.push(applied_force as f64); // Fix: Cast f32 (Real) to f64
            }
        }
